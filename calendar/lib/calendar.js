...
(function(window, document, LunarCalendar, undefined){
...
Calendar.prototype._updateMonthPane = function($monthPane, curtDate, today){
	var $dateCells = this._getDateCells($monthPane)
	  ...
	  , labels     = this._getMonthLabels(curtDate);
	...
	for( var i = 0, len = $dateCells.length; i < len; ++i ){
		 ...
		 this._updateDateCell($dateCell, date, labels[i]);
	}
...
Calendar.prototype._updateDateCell = function($dateCell, date, label){
...
Calendar.prototype._getLunarDates = function(date){
	var row           = 6
	  , col           = 7
	  , len           = row * col
	  , dates         = new Array(len)
	...
	curtMonthLunarDates = LunarCalendar.calendar(curtMonthYear, curtMonthMonth + 1, false).monthData;
	for( var i = 0; i < curtMonthNum; ++i ){
		 dates[i + firstDay] = curtMonthLunarDates[i]; 
	}
	...
	prevMonthLunarDates = LunarCalendar.calendar(prevMonthYear, prevMonthMonth + 1, false).monthData;

	for( var i = 0; i < firstDay; ++i ){
		 dates[i] = prevMonthLunarDates[prevMonthNum - firstDay + i];
	}
	...
	nextMonthLunarDates = LunarCalendar.calendar(nextMonthYear, nextMonthMonth + 1, false).monthData;

	for( var i = curtMonthNum + firstDay; i < len; ++i ){
		 dates[i] = nextMonthLunarDates[i - curtMonthNum - firstDay];
	}
	
	return dates;
};

Calendar.prototype._getMonthLabels = function(date){
	var dates  = this._getLunarDates(date)
	  , len    = dates.length
	...
	for( i = 0; i < len; ++i ){
		 ...
		 labels[i] = lunarDay == 1 ? lunarMonthName : (term || lunarDayName);
	}
...
Calendar.prototype.update = function(dateToSelect, animate){
	...
	$prevMonthPane.style.transitionDuration = '0s';
	$curtMonthPane.style.transitionDuration = '0s';
	$nextMonthPane.style.transitionDuration = '0s';

	$prevMonthPane.style.webkitTransitionDuration = '0s';
	$curtMonthPane.style.webkitTransitionDuration = '0s';
	$nextMonthPane.style.webkitTransitionDuration = '0s';

	if( animate ){
		if( tMY === cMY && tMM == cMM ){
			offset = getTranslateX($curtMonthPane);

			durations[0] = getDuration(Math.abs(offset));
			...
			durations[0] = 0;
			durations[1] = getDuration(Math.abs(offset));
			durations[2] = getDuration(Math.abs(offset));
	...
	$prevMonthPane.style.transitionDuration = durations[0] + 's';
	$curtMonthPane.style.transitionDuration = durations[1] + 's';
	$nextMonthPane.style.transitionDuration = durations[2] + 's';

	$prevMonthPane.style.webkitTransitionDuration = durations[0] + 's';
	$curtMonthPane.style.webkitTransitionDuration = durations[1] + 's';
	$nextMonthPane.style.webkitTransitionDuration = durations[2] + 's';
